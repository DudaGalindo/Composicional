...
class StabilityCheck:
    ...
    def run(self, z, Mw):
        ...
        self.A = np.zeros(len(self.P))     # 3 phases
        self.x = np.empty(z.shape)
        self.y = np.empty(z.shape)
        self.a = np.zeros(z.shape)         # 3 phases
    ...
    def solve_objective_function_Whitson_for_V_and_A(self, z, V, A, ponteiro):
        ...
        while any(ponteiro):
            ...
            Jacobian = np.zeros([ponteiro[ponteiro].size, 2, 2])
            Jacobian[:,0,0] = df_VdV
            Jacobian[:,0,1] = df_VdA
            Jacobian[:,1,0] = df_AdV
            Jacobian[:,1,1] = df_AdA
    ...
    def molar_properties_Lapene_3phase(self, PR, z, ponteiro):
        ...
        while any(ponteiro):
            ...
            except: import pdb; pdb.set_trace() #print('Solve for V a classical RR equation')
            '''
            ...
            lnphiv = self.lnphi_based_on_deltaG(PR, y, self.P[ponteiro], self.ph_V[ponteiro])
            lnphia = self.lnphi_based_on_deltaG(PR, self.a[:,ponteiro], self.P[ponteiro], self.ph_L[ponteiro])


            self.fv = np.exp(lnphiv) * (y * self.P[ponteiro][np.newaxis,:])
            self.fl = np.exp(lnphil) * (x * self.P[ponteiro][np.newaxis,:])
            self.fa = np.exp(lnphia) * (self.a[:,ponteiro] * self.P[ponteiro][np.newaxis,:])
